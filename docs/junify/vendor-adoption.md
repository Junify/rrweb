...
| P0 | same-batch and pre-observer-flush dynamic password masking: Task 6 differential against upstream 2.1.1 | no | `adapt` | both same-batch type/value mutation orders plus post-start and temporary password Input before observer flush remain masked, without stale masking afterward | Task 6 plus fix round 1 package-local `G-PRIVACY`; ledger `PRIV-004`; Task 9 storage/V1/V2 residual | upstream correctness candidate; delete when upstream stable passes all focused lifecycle partitions without the patch |
